...
const SearchResults = () => {
  ...
  const gameKeys = Object.keys(translated).filter(
    (key) => typeof translated[key] === "string" && translated[key].length > 0,
  );

  if (gameKeys.length == 0 || query.isLoading || query.isFetching) return null;
