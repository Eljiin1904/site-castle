...
export const BlackjackSoundManager = memo(() => {
  ...
  useEffect(() => {
    ...
    const cardsDealtFn = () => {
      ...
      const hasNewSidebets = sidebetPayouts.some(
        (payout) => payout.amount > 0 && !payout.fromData,
      );
      if (hasNewSidebets) {
      ...
      const someHandsWon = player.hands.some(
        (hand) => hand.mainPayout?.result === "win",
      );
      const someHandsTie = player.hands.some(
        (hand) => hand.mainPayout?.result === "tie",
      );
