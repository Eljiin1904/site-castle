...
import { CrashInfoModal } from "#app/modals/crash/CrashInfoModal";

export const CrashHeader = () => {
      ...
      >
        {!small && <Fragment>
          <Button
            ...
            onClick={() => Dialogs.open("primary", <FairnessSeedModal historyTo="/fairness/crash" />)}
          />
          <Button
            kind="tertiary-black-overlay"
            icon={SvgInfoCircle}
            onClick={() => Dialogs.open("primary", <CrashInfoModal />)}
          />
        </Fragment>}
        <VolumeDropdown prefix="crash" />
        {small && <CrashHeaderMenu />}
      </Div>
    </Div>
  );
};
