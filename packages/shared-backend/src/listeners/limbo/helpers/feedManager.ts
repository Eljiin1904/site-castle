import { TypedEventEmitter } from "@core/services/utility";
import { LimboTicketDocument } from "@core/types/limbo/LimboTicketDocument";
import { Database } from "@server/services/database";
import { System } from "@server/services/system";
import { Limbo } from "@server/services/limbo";
import { ticketStream } from "./ticketStream";

export class FeedManager extends TypedEventEmitter<{
  initialized: () => void;
  insert: (document: LimboTicketDocument) => void;
}> {
  private _log: LimboTicketDocument[] = [];
  private readonly _queue: LimboTicketDocument[] = [];
  private _initialized = false;

  constructor() {
    super();

    System.tryCatch(this.init.bind(this))();

    ticketStream.on("insert", System.tryCatch(this.onInsert.bind(this)));

    setInterval(System.tryCatch(this.onInterval.bind(this)), 600);
  }

  get log() {
    return this._log;
  }

  waitForInit() {
    if (this._initialized) {
      return;
    }
    return new Promise((resolve) => this.once("initialized", resolve));
  }

  private async init() {
    await Database.manager.waitForInit();

    this._log = await Database.collection("limbo-tickets")
      .find(
        {
          betAmount: { $gt: 0 },
        },
        {
          sort: { timestamp: -1 },
          limit: Limbo.feedLogSize,
        },
      )
      .toArray();

    this._initialized = true;
    this.emit("initialized");
  }

  private async onInsert(document: LimboTicketDocument) {
    if (document.betAmount === 0) {
      return;
    }

    this._queue.unshift(document);

    if (this._queue.length > Limbo.feedLogSize) {
      this._queue.length = Limbo.feedLogSize;
    }
  }

  private async onInterval() {
    const document = this._queue.pop();

    if (document) {
      const length = this._log.unshift(document);

      if (length > Limbo.feedLogSize) {
        this._log.length = Limbo.feedLogSize;
      }

      this.emit("insert", document);
    }
  }
}

export const feedManager = new FeedManager();
