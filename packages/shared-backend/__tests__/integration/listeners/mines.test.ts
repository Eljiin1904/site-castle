...
import { createTestTicket, createTestUser, handleLogin, fetchWithCookie } from "../../testUtility";
import { io, Socket } from "socket.io-client";
import config from "#app/config";
import { DiceTicketDocument } from "@core/types/dice/DiceTicketDocument";
import bcrypt from "bcrypt";
import { message } from "@core/services/validation/Validation";
...
describe("Mines Game Test ", async () => {
  it("Join Mines Game Feed", async () => {
    ...
    const user = await Database.collection("users").findOne({ username: "minesListenerTest" });
