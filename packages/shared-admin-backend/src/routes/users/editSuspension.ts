...
export default Http.createApiRoute({
  ...
  callback: async (req, res) => {
    ...
    const user = await Database.collection("users").findOneAndUpdate(
      ...
      {
        $set: {
          "suspension.reason": enabled ? reason : null,
          "suspension.startDate": enabled ? new Date() : null,
          "suspension.endDate": enabled ? endDate : null,
        },
      },
    );
